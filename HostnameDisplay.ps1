...
# Move the form lower so the text is just above the taskbar
$taskbarHeight = 10  # Adjust as needed for your taskbar size
$x = $screenBounds.Right - $form.Width - $paddingRight
$y = $screenBounds.Bottom - $form.Height + $taskbarHeight - $paddingBottom
...
$form.Add_Paint({
    ...
    $y = $form.ClientSize.Height - $size.Height - 1 # 1 pixel above the bottom
...
$timer.Add_Tick({
    ...
    # Sample just above the text (match the Y used in the paint event)
    $sampleMargin = 10  # Same as the margin in your paint event
    ...
    $script:sampleY = $form.ClientSize.Height - $size.Height - $sampleMargin
